...
void radius(void)
{
    ...
    if ((fpin = fopen(pfilename, "r")) == NULL) {
        printf("\n Cannot open parameter file (pfilename): %s in radius(), exit",
               pfilename);
        exit(1);
    }
    if (radiusindex == 1) {
...
/*     printf("\n Cannot open file %s, exit", rfilename);*/
        ...
        if ((fpout = fopen(rfilename, "a")) == NULL) {
            printf("\n Cannot open radius file (rfilename): %s in radius(), exit",
                   rfilename);
            exit(1);
        }
    ...
    for (i = 0; i < atomnum; i++) {
        ...
        if (radiusindex == 1)
            fprintf(fpout, "%-5s%-5s%8.5lf\n", atom[i].name, atom[i].aa, atom[i].radius);
...
void wdelc(char *filename)
{
    ...
    if ((fpout = fopen(filename, "a")) == NULL) {
        printf("\n Cannot open a file (filename) to append: %s in wdelc(), exit",
               filename);
        exit(1);
    }
    ...
    for (i = 0; i < atomnum; i++)
        fprintf(fpout, "%-6s%c%c%c%13.5lf\n", atom[i].name, atom[i].aa[0], atom[i].aa[1],
                atom[i].aa[2], atom[i].charge);
    ...
    if (strcmp(COLORTEXT, "YES") == 0 || strcmp(COLORTEXT, "yes") == 0) {
        ...
        if (argc == 2)
            if (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "-H") == 0) {
                ...
                printf("                 -m  modified pdb file name (optional)\n");
                printf("                 -p  radius parameter file name (optional)\n");
                printf
