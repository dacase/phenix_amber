    ...
    for (;;) {
        ...
        if (strncmp("PARM", line, 4) == 0) {
            ...
            if (ngeomparm >= MAXGEOMPARM) {
                eprintf("The number of parameters (%d) exceeded MAXGEOMPARM.\n"
                        "Increase MAXGEOMPARM in equatom2.c and rebuild.", ngeomparm);
            }
        }
    }
    fclose(fp);
    ...
    for (i = 0; i < bondnum; i++) {
        ...
        if (suc == 1) {
            for (m = 0; m < 6; m++) {
                if (atom[at2].con[m] < 0)
                    break;
                if (atom[at2].con[m] == at3)
                    continue;
                at1 = atom[at2].con[m];
                for (n = 0; n < 6; n++) {
                    if (atom[at3].con[n] < 0)
                        break;
                    if (atom[at3].con[n] == at2)
                        continue;
                    at4 = atom[at3].con[n];
                    if (at1 < at4) {
                        geom[count].id1 = at1;
                    ...
                    count++;
                    if (count >= MAXGEOM) {
                        eprintf("The number of E/Z configurations (%d) exceeded MAXGEOM.\n"
                                "Increase MAXGEOM in equatom2.c and rebuild.", count);
                    }
                }
            }
        }
...
void scorepath_input(ATOM atom[], int selectnum, int startnum, int max_path_length,
                     int ngeom, GEOM geom[])
{
    int i, j, k, m;
    int start;
        ...
        if (atom[startnum].con[i] == -1) {
            ...
            for (j = 0; j < selectnum; j++) {
                ...
                if (ngeom > 0) {
                    ...
                    for (m = 0; m < ngeom; m++) {
                        if ((selectnum - j) > 3
                            && input_selectelement[j + 3] == geom[m].id4
                            && input_selectelement[j + 2] == geom[m].id3
                            && input_selectelement[j + 1] == geom[m].id2
                            && input_selectelement[j] == geom[m].id1) {
                            if (geom[m].type == 1)
                        ...
                        if ((selectnum - j) > 3
                            && input_selectelement[j + 3] == geom[m].id1
                            && input_selectelement[j + 2] == geom[m].id2
                            && input_selectelement[j + 1] == geom[m].id3
                            && input_selectelement[j] == geom[m].id4) {
                            if (geom[m].type == 1)
            ...
            if (pathnumindex >= input_pathatomnum[atomindex]) {
                ...
                printf("Info: The number of path atoms exceeded MAXPATHATOMNUM"
                       " for atom (ID: %d, Name: %s).\n"
                       "      Automatically increasing to %d.\n",
                       atomindex, atom[atomindex].name, input_pathatomnum[atomindex]);
                input_pathscore[atomindex] = (double *) erealloc(input_pathscore[atomindex],
                                       input_pathatomnum[atomindex] * sizeof(double));
            }
            return;
        }
...
void scorepath_ref(ATOM atom[], int selectnum, int startnum, int max_path_length,
                   int ngeom, GEOM geom[])
{
    int i, j, k, m;
    int start;
    ...
    for (i = 0; i < 6; i++) {
        if (atom[startnum].con[i] == -1) {
            ...
            for (j = 0; j < selectnum; j++) {
                ...
                score +=
                    coef * ((j + 1) * 0.11 + atom[ref_selectelement[j]].atomicnum * 0.08);
            }
            ...
            if (pathnumindex >= ref_pathatomnum[atomindex]) {
                ...
                printf("Info: The number of path atoms exceeded MAXPATHATOMNUM"
                       " for atom (ID: %d, Name: %s).\n"
                       "      Automatically increasing to %d.\n",
                       atomindex, atom[atomindex].name, ref_pathatomnum[atomindex]);
                ref_pathscore[atomindex] = (double *) erealloc(ref_pathscore[atomindex],
                                       ref_pathatomnum[atomindex] * sizeof(double));
            }
            return;
        }
        start = atom[startnum].con[i];
...
void match_atompair(int input_atomnum, int ref_atomnum, ATOM * input_atom,
                    ATOM * ref_atom, int *input_equ_atom_id, int *ref_equ_atom_id,
                    int *input_atom_id, int *ref_atom_id, int input_bondnum,
                    int ref_bondnum, BOND * input_bond, BOND * ref_bond,
                    int max_path_length, int igeom)
{
    int i, j, k, m, n;
    int input_ngeom = 0;
    int ref_ngeom = 0;
...
/* allocation memory */
    if (input_atomnum > MAX_EQU_ATOM) {
        eprintf("The number of atoms (%d) exceeded MAX_EQU_ATOM.\n"
                "Increase MAX_EQU_ATOM in equatom2.c and rebuild.",
                input_atomnum);
    }
    ...
    input_selectindex = (int *) emalloc(sizeof(int) * input_atomnum);
    ref_selectindex = (int *) emalloc(sizeof(int) * ref_atomnum);

    input_pathnum = (int *) emalloc(sizeof(int) * input_atomnum);
    ref_pathnum = (int *) emalloc(sizeof(int) * ref_atomnum);

    input_selectelement = (int *) emalloc(sizeof(int) * input_atomnum);
    ref_selectelement = (int *) emalloc(sizeof(int) * ref_atomnum);

    input_pathatomnum = (int *) emalloc(sizeof(int) * input_atomnum);
    input_pathscore = (double **) emalloc(MAX_EQU_ATOM * sizeof(double *));
    for (i = 0; i < input_atomnum; i++) {
        input_pathatomnum[i] = MAXPATHATOMNUM;
        input_pathscore[i] = (double *) ecalloc(input_pathatomnum[i], sizeof(double));
    }
    ref_pathatomnum = (int *) emalloc(sizeof(int) * ref_atomnum);
    ref_pathscore = (double **) emalloc(MAX_EQU_ATOM * sizeof(double *));
    for (i = 0; i < ref_atomnum; i++) {
        ref_pathatomnum[i] = MAXPATHATOMNUM;
        ref_pathscore[i] = (double *) ecalloc(ref_pathatomnum[i], sizeof(double));
    }
    ...
    if (igeom == 1) {
        ...
        ref_ngeom = 0;
        if (ngeomparm > 0) {
            input_geom = (GEOM *) emalloc(sizeof(GEOM) * MAXGEOM);
            geometry(input_atom, input_atomnum, input_bond, input_bondnum, input_geom,
                     &input_ngeom);

            ref_geom = (GEOM *) emalloc(sizeof(GEOM) * MAXGEOM);
