...
int main(int argc, char *argv[])
{
    ...
    if (format != 0 && format != 1 && format != 2) {
        eprintf("Invalid file format, must be 'ac', 'pdb' or 'mol2'.");
    }
    ...
    if (strcmp(command, "check") == 0) {
        ...
        printf("\nThe molecule center is at: X= %9.4lf, Y= %9.4lf, Z = %9.4lf", coordx,
               coordy, coordz);
    ...
    if (strcmp(command, "translate") == 0) {
        ...
        if (vecty < -99990) {
            eprintf("No -vy specified.");
        }
        if (vectz < -99990) {
            eprintf("No -vz specified.");
        }
        coordx = vectx;
        coordy = vecty;
        coordz = vectz;
    ...
    if (strcmp(command, "match") == 0) {
        ...
        ref_atom = (ATOM *) emalloc(sizeof(ATOM) * cinfo.maxatom);
        ref_bond = (BOND *) emalloc(sizeof(BOND) * cinfo.maxbond);
        int i;
        for (i = 0; i < cinfo.maxbond; ++i) {
        ...
        if (format == 1)
            overflow_flag = rpdb(rfilename, &atomnum, ref_atom, cinfo, minfo, 0);
        if (format == 2)
            overflow_flag =
                rmol2(rfilename, &atomnum, ref_atom, &bondnum, ref_bond, &cinfo, &minfo,
                      1);
