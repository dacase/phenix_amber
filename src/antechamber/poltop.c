    ...
    for (;;) {
        ...
        if (read_flag == 1) {
            ...
            if (strncmp(line, "PARM", 4) == 0) {
                ...
                sscanf(&line[4], "%s%lf%lf", parm[parmnum].name, &parm[parmnum].pol,
                       &parm[parmnum].damp_factor);
...
void rcorr()
{
    ...
    for (;;) {
        if (fgets(line, MAXCHAR, fptype) == NULL)
            break;
        if (strncmp(line, "CORR", 4) == 0) {
            ...
            if (suc == 0) {
                fprintf(stdout,
                        "The atom type (%s), which is corresponding to (%s),\n"
                        "is not in the parm file (%s).\n",
...
void wtop()
{
    ...
    for (;;) {
        ...
        if (write_flag == 0 && strcmp("%FLAG", tmpchar1) == 0)
            write_flag = 1;
        if (strcmp("%FLAG", tmpchar1) == 0 && strcmp("DIPOLE_DAMP_FACTOR", tmpchar2) == 0) {
            strcpy(tmpchar1, "");
            tmpchar1[0] = '\0';
...
// find the parameters
    for (i = 0; i < atomnum; i++) {
        ...
        if (suc == 0) {
            fprintf(stdout,
                    "The atom type (%s) is not in the atom type file (%s).\n",
                    atom[i].type, tfilename);
            exit(0);
        }
        if (debug == 1) {
            printf("ATOM %d %5s %5s %9.5lf %9.5lf\n", i + 1, atom[i].name, atom[i].type,
                   atom[i].pol, atom[i].damp_factor);
        }
    }

    if (atom[0].damp_factor < -999990)
        idamp_factor = 0;
