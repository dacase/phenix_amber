...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(COLORTEXT, "YES") == 0 || strcmp(COLORTEXT, "yes") == 0) {
        ...
        if (argc == 2 && (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "-H") == 0)) {
            ...
            printf("               -f prepin file format: prepi (default) or prepc\n\n");
            exit(1);
        }
        if (argc != 9) {
            printf("Usage: crdgrow -i input file name (pdb)\n");
            printf("               -o output file name (pdb)\n");
            printf("               -p prepin file name\n");
            printf("               -f prepin file format (prepi or prepc)\n\n");
            exit(1);
        }
    }
    tmpchar[0] = '\0';
...
/* memory allocation*/
    default_cinfo(&cinfo);
    default_minfo(&minfo);
    atom1 = (ATOM *) emalloc(sizeof(ATOM) * cinfo.maxatom);
    atom2 = (ATOM *) emalloc(sizeof(ATOM) * cinfo.maxatom);
    atom3 = (ATOM *) emalloc(sizeof(ATOM) * cinfo.maxatom);
    ...
    if (format != 0)
        connect(connect_file, atomnum, atom1, &bondnum_tmp, bond_tmp, cinfo.maxbond);
    ...
    for (i = 0; i < atomnum; i++)
        if (index[i] == 0) {
            ...
            for (j = 0; j < atom1[ref2].connum; j++)
                if (index[atom1[ref2].con[j]] == 1 && atom1[ref2].con[j] != ref1) {
            ...
            torsion = rotate(atom1[i], atom1[ref1], atom1[ref2], &atom1[ref3]);
            torsion1 = NEGTOR;
            torsion2 = NEGTOR;
            torsion3 = NEGTOR;
            torsion4 = NEGTOR;
            ...
            if (i != tmpint1 && ref2 != tmpint1 && tmpint1 != -1) {
                if (index[tmpint1] == 1)
                    torsion1 =
                        rotate(atom3[tmpint1], atom3[ref1], atom3[ref2], &atom3[ref3]);
                torsion1a =
                    rotate(atom1[tmpint1], atom1[ref1], atom1[ref2], &atom1[ref3]);
            }
            tmpint2 = atom1[ref1].con[1];
            if (i != tmpint2 && ref2 != tmpint2 && tmpint2 != -1) {
                if (index[tmpint2] == 1)
                    torsion2 =
                        rotate(atom3[tmpint2], atom3[ref1], atom3[ref2], &atom3[ref3]);
                torsion2a =
                    rotate(atom1[tmpint2], atom1[ref1], atom1[ref2], &atom1[ref3]);
            }
            tmpint3 = atom1[ref1].con[2];
            if (i != tmpint3 && ref2 != tmpint3 && tmpint3 != -1) {
                if (index[tmpint3] == 1)
                    torsion3 =
                        rotate(atom3[tmpint3], atom3[ref1], atom3[ref2], &atom3[ref3]);
                torsion3a =
                    rotate(atom1[tmpint3], atom1[ref1], atom1[ref2], &atom1[ref3]);
            }
            tmpint4 = atom1[ref1].con[3];
            if (i != tmpint4 && ref2 != tmpint4 && tmpint4 != -1) {
                if (index[tmpint4] == 1)
                    torsion4 =
                        rotate(atom3[tmpint4], atom3[ref1], atom3[ref2], &atom3[ref3]);
                torsion4a =
                    rotate(atom1[tmpint4], atom1[ref1], atom1[ref2], &atom1[ref3]);
            }
            tmpint5 = atom1[ref1].con[4];
            if (i != tmpint5 && ref2 != tmpint5 && tmpint5 != -1) {
                if (index[tmpint5] == 1)
                    torsion5 =
                        rotate(atom3[tmpint5], atom3[ref1], atom3[ref2], &atom3[ref3]);
                torsion5a =
                    rotate(atom1[tmpint5], atom1[ref1], atom1[ref2], &atom1[ref3]);
            }
            ...
            if (num >= 1) {
                ...
                torsion0 = torsion;
            rotate0(atom3[ref3], atom3[ref2], atom3[ref1], &atom3[i], bond, angle,
                    torsion0);
            index[i] = 1;
        }
