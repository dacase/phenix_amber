        ...
        if (strncmp("PARM", line, 4) == 0) {
            ...
            if (ngeomparm >= MAXGEOMPARM) {
                eprintf("The number of parameters (%d) exceeded MAXGEOMPARM.\n"
                        "Increase MAXGEOMPARM in equatom.c and rebuild.", ngeomparm);
            }
        }
    }
    fclose(fp);
                ...
                for (n = 0; n < 6; n++) {
                    ...
                    if (count >= MAXGEOM) {
                        eprintf("The number of E/Z configurations (%d) exceeded MAXGEOM.\n"
                                "Increase MAXGEOM in equatom.c and rebuild.", count);
                    }
                }
            }
        }
...
void scorepath(ATOM atm[], int selectnum, int startnum, int max_path_length, int ngeom,
               GEOM geom[])
{
    int i, j, k, m;
    int start;
    ...
    for (i = 0; i < 6; i++) {
        if (atm[startnum].con[i] == -1) {
            ...
            for (j = 0; j < selectnum; j++) {
                ...
                score += coef * ((j + 1) * 0.11 + atm[selectelement[j]].atomicnum * 0.08);
            }
            ...
            if (pathnumindex >= pathatomnum[atomindex]) {
                ...
                printf("Info: The number of path atoms exceeded MAXPATHATOMNUM"
                       ...
                       atomindex, atm[atomindex].name, pathatomnum[atomindex]);
                pathscore[atomindex] = (double *) erealloc(pathscore[atomindex],
                                       pathatomnum[atomindex] * sizeof(double));
            }
            return;
        }
...
void equatom(int atomnum, ATOM * atom, int *equ_atom_id, int max_path_length, int ngeom,
             GEOM geom[])
{
    int i, j, k;
    int equindex;
...
void identify_equatom(int atomnum, ATOM * atom, int *equ_atom_id, int max_path_length,
                      int bondnum, BOND * bond, int igeom)
{
    int i;
    int ngeom;
    GEOM *geom = NULL;
    if (atomnum > MAX_EQU_ATOM) {
        eprintf("The number of atoms (%d) exceeded MAX_EQU_ATOM.\n"
                "Increase MAX_EQU_ATOM in equatom.c and rebuild.", atomnum);
    }

    selectindex = (int *) emalloc(sizeof(int) * atomnum);
    pathnum = (int *) emalloc(sizeof(int) * atomnum);
    selectelement = (int *) emalloc(sizeof(int) * atomnum);
    pathatomnum = (int *) emalloc(sizeof(int) * atomnum);
    if (atomnum > MAX_EQU_ATOM) {
        eprintf("The number of atoms (%d) exceeded MAX_EQU_ATOM.\n"
                "Increase MAX_EQU_ATOM in equatom.c and rebuild.", atomnum);
    }
    for (i = 0; i < atomnum; i++) {
        ...
        pathscore[i] = (double *) ecalloc(pathatomnum[i], sizeof(double));
    }
    ngeom = 0;
    if (igeom == 1) {
        geom = (GEOM *) emalloc(sizeof(GEOM) * MAXGEOM);
