...
void ratcorr(void)
{
    corratnum = 0;
    for (;;) {
        if (fgets(line, MAXCHAR, fpat) == NULL)
            break;
        sscanf(line, "%s%s", atorg[corratnum], atcorr[corratnum]);
        corratnum++;
        if (corratnum > MAX_AT_CORR) {
            eprintf("The number of corresponding atom type pairs (%d) exceeded MAX_AT_CORR.\n"
                    "Increase MAX_AT_CORR in top2mol2.c and rebuild.", corratnum);
        }
    }
}
...
void rbtcorr(void)
{
    corrbtnum = 0;
    for (;;) {
        if (fgets(line, MAXCHAR, fpbt) == NULL)
            break;
        sscanf(line, "%s%s%s", btan1[corrbtnum], btan2[corrbtnum], bt[corrbtnum]);
        corrbtnum++;
        if (corrbtnum > MAX_BT_CORR) {
            eprintf("The number of corresponding bond type pairs (%d) exceeded MAX_BT_CORR.\n"
                    "Increase MAX_BT_CORR in top2mol2.c and rebuild.", corrbtnum);
        }
    }
}
...
void rcrd()
{
    ...
    j = 0;
    for (;;) {
        if (fgets(line, MAXCHAR, fpinc) == NULL)
            break;
        j++;
        if (j == 2) {
            sscanf(line, "%ld", &crd_atomnum);
            if (crd_atomnum != top_atomnum) {
                eprintf("Mismatching atom numbers from the topology (%ld) and the crd (%ld) files.",
                        top_atomnum, crd_atomnum);
            }
        }
        if (strncmp(".", &line[4], 1) == 0 && strncmp(".", &line[16], 1) == 0
            && strncmp(".", &line[28], 1) == 0) {
            sscanf(&line[1], "%lf%lf%lf%lf%lf%lf", &coord_x[i], &coord_y[i], &coord_z[i],
...
void rtop(int mode)
{
    ...
    for (;;) {
        ...
        line[71] = ' ';
        line[79] = ' ';
        if (fgets(line, MAXCHAR, fpinp) == NULL)
            break;
        sscanf(line, "%s%s", tmpchar1, tmpchar2);
        if (strcmp("%FLAG", tmpchar1) == 0
            && (flag == 1 || flag == 3 || flag == 5 || flag == 7 || flag == 9
                || flag == 11 || flag == 13 || flag == 15))
            flag = -999;
        ...
        if (mode == 0 && flag == 1) {
            sscanf(line, "%ld%ld%ld%ld", &top_atomnum, &tmpint, &top_bondnum_h,
                   &top_bondnum_noh);
        ...
        if (flag == 5) {
            sscanf(line, "%lf%lf%lf%lf%lf", &charge[i], &charge[i + 1], &charge[i + 2],
                   &charge[i + 3], &charge[i + 4]);
            charge[i] /= 18.2223;
            charge[i + 1] /= 18.2223;
            charge[i + 2] /= 18.2223;
            charge[i + 3] /= 18.2223;
        ...
        if (flag == 9) {
            sscanf(line, "%ld%ld%ld%ld%ld%ld%ld%ld%ld%ld", &reshead[i], &reshead[i + 1],
                   &reshead[i + 2], &reshead[i + 3], &reshead[i + 4], &reshead[i + 5],
                   &reshead[i + 6], &reshead[i + 7], &reshead[i + 8], &reshead[i + 9]);
            i += 10;
        }
        if (flag == 11) {
            j = 0;
...
static void write(void)
{
    ...
    fprintf(fpout, "%s\n", "MOL");
    if (water_flag == 0)
        fprintf(fpout, "%5ld%6ld%6ld%6d%6d\n", atomnum - 3 * resnum_water,
                bondnum - 3 * resnum_water, resnum - resnum_water, 0, 0);
    else
    ...
    for (i = 0; i < atomnum; i++) {
        ...
        fprintf(fpout, "%7ld %-8s%10.4lf%10.4lf%10.4lf %-6s%5ld %-8s%10.4lf\n", tmpint,
                atomname[i].str, coord_x[i], coord_y[i], coord_z[i], atomtype[i].str,
                resno[i], tmpc2, charge[i]);
    }
    fprintf(fpout, "@<TRIPOS>BOND\n");
    tmpint = 0;
    for (i = 0; i < bondnum_noh; i++) {
        tmpint++;
        fprintf(fpout, "%6ld %5ld %5ld %-4s\n", tmpint, bondat1_noh[i] + 1,
                bondat2_noh[i] + 1, bondtype_noh[i].str);
    }

    for (i = 0; i < bondnum_h; i++) {
        if (water_flag == 0) {
            if (strcmp(atomres[bondat1_h[i]].str, "WAT") == 0)
                continue;
            if (strcmp(atomres[bondat2_h[i]].str, "WAT") == 0)
                continue;
        }
        tmpint++;
        fprintf(fpout, "%6ld %5ld %5ld %-4s\n", tmpint, bondat1_h[i] + 1,
                bondat2_h[i] + 1, bondtype_h[i].str);
    }
...
/*  readin a topology file and crd file,then save out a mol2 file */
    ...
    fpinp = efopen(pfilename, "r");
    fpinc = efopen(cfilename, "r");
    fpout = efopen(ofilename, "w");
    if (output_atomtype_flag == 2)
        fpat = efopen(atfilename, "r");
    if (output_bondtype_flag == 2)
        fpbt = efopen(btfilename, "r");

    rtop(0);
    rewind(fpinp);

    bondat1_h = (long *) emalloc(sizeof(long) * (top_bondnum_h + 10));
    bondat2_h = (long *) emalloc(sizeof(long) * (top_bondnum_h + 10));
    bondat1_noh = (long *) emalloc(sizeof(long) * (top_bondnum_noh + 10));
    bondat2_noh = (long *) emalloc(sizeof(long) * (top_bondnum_noh + 10));
    resno = (long *) emalloc(sizeof(long) * (top_atomnum + 10));
    reshead = (long *) emalloc(sizeof(long) * (top_resnum + 10));
    charge = (double *) emalloc(sizeof(double) * (top_atomnum + 10));
    coord_x = (double *) emalloc(sizeof(double) * (top_atomnum + 10));
    coord_y = (double *) emalloc(sizeof(double) * (top_atomnum + 10));
    coord_z = (double *) emalloc(sizeof(double) * (top_atomnum + 10));
    res = (STR_ARRAY *) emalloc(sizeof(STR_ARRAY) * (top_resnum + 10));
    bondtype_h = (STR_ARRAY *) emalloc(sizeof(STR_ARRAY) * (top_bondnum_h + 10));
    bondtype_noh = (STR_ARRAY *) emalloc(sizeof(STR_ARRAY) * (top_bondnum_noh + 10));
    atomname = (STR_ARRAY *) emalloc(sizeof(STR_ARRAY) * (top_atomnum + 10));
    atomtype = (STR_ARRAY *) emalloc(sizeof(STR_ARRAY) * (top_atomnum + 10));
    atomtypebak = (STR_ARRAY *) emalloc(sizeof(STR_ARRAY) * (top_atomnum + 10));
    atomres = (STR_ARRAY *) emalloc(sizeof(STR_ARRAY) * (top_atomnum + 10));
...
*/
    ...
    for (i = 0; i < bondnum_h; i++) {
        ...
        if (output_bondtype_flag == 2) {
            for (j = 0; j < corrbtnum; j++) {
                if (strcmp(atomtypebak[bondat1_h[i]].str, btan1[j]) == 0
                    && strcmp(atomtypebak[bondat2_h[i]].str, btan2[j]) == 0) {
                    strcpy(bondtype_h[i].str, bt[j]);
                    break;
                }
                if (strcmp(atomtypebak[bondat1_h[i]].str, btan2[j]) == 0
                    && strcmp(atomtypebak[bondat2_h[i]].str, btan1[j]) == 0) {
                    strcpy(bondtype_h[i].str, bt[j]);
                    break;
                }
            }
    ...
    for (i = 0; i < bondnum_noh; i++) {
        ...
        if (output_bondtype_flag == 2) {
            for (j = 0; j < corrbtnum; j++) {
                if (strcmp(atomtypebak[bondat1_noh[i]].str, btan1[j]) == 0
                    && strcmp(atomtypebak[bondat2_noh[i]].str, btan2[j]) == 0) {
                    strcpy(bondtype_noh[i].str, bt[j]);
                    break;
                }
                if (strcmp(atomtypebak[bondat1_noh[i]].str, btan2[j]) == 0
                    && strcmp(atomtypebak[bondat2_noh[i]].str, btan1[j]) == 0) {
                    strcpy(bondtype_noh[i].str, bt[j]);
                    break;
                }
            }
