...
pysander_InputOptions_dealloc(pysander_InputOptions* self) {
    ...
    Py_DECREF(self->ntc);
    Py_DECREF(self->ntr);
    Py_DECREF(self->ibelly);
    Py_DECREF(self->mask_from_ref);
...
pysander_InputOptions_new(PyTypeObject *type) {
    ...
    if (self != NULL) {
        ...
        self->ibelly = PyInt_FromLong(0);
        self->mask_from_ref = PyInt_FromLong(0);
...
static PyMemberDef pysander_InputOptionMembers[] = {
    ...
    {"ibelly", T_OBJECT_EX, offsetof(pysander_InputOptions, ibelly), 0,
                "Flag to set whether belly are used"},
    {"mask_from_ref", T_OBJECT_EX, offsetof(pysander_InputOptions, mask_from_ref), 0,
                "Flag to set whether mask uses reference coords"},
    ...
    {"bellymask", T_OBJECT_EX, offsetof(pysander_InputOptions, bellymask), 0,
                "Mask string selecting the atoms for belly"},

    {"refc", T_OBJECT_EX, offsetof(pysander_InputOptions, refc), 0,
                "reference filename"},
