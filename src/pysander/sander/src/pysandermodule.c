...
pysander_setup(PyObject *self, PyObject *args) {
    ...
    input.ibelly = (int) PyInt_AsLong(mm_inp->ibelly);
    input.mask_from_ref = (int) PyInt_AsLong(mm_inp->mask_from_ref);
    ...
    if (!PyObject_IS_STRING(mm_inp->bellymask)) {
        PyErr_SetString(PyExc_ValueError, "bellymask must be a string");
        return NULL;
    }
    if (PyString_Size(mm_inp->bellymask) >= 256) {
        PyErr_SetString(PyExc_ValueError,
                        "bellymask must be smaller than 256 characters");
        return NULL;
    } else {
        strncpy(input.bellymask, PyString_AsString(mm_inp->bellymask),
                PyString_Size(mm_inp->bellymask));
        for (i = PyString_Size(mm_inp->bellymask); i < 256; i++)
            input.bellymask[i] = ' ';
    }

    if (!PyObject_IS_STRING(mm_inp->refc)) {
        PyErr_SetString(PyExc_ValueError, "refc must be a string");
        return NULL;
    }
    if (PyString_Size(mm_inp->refc) >= 256) {
        PyErr_SetString(PyExc_ValueError,
                        "refc must be smaller than 256 characters");
        return NULL;
    } else {
        strncpy(input.refc , PyString_AsString(mm_inp->refc),
                PyString_Size(mm_inp->refc));
        for (i = PyString_Size(mm_inp->refc); i < 256; i++)
            input.refc[i] = ' ';
    }
